// Version: 3.6.0 - Final Sound Fix & Balance
let gold = 1000;
...
function saveGame() {
    ...
    localStorage.setItem('toothSaveV360', JSON.stringify(data));
}

function loadGame() {
    const saved = localStorage.getItem('toothSaveV360');
    if (saved) {
        ...
        if (!isMiningPaused && d.lastTime) {
            ...
            const miningSpeed = Math.max(7, 15 - (autoMineLevel * 0.2)); // 밸런스 수정
