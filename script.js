// Version: 2.4.0 - Refine Tab & Hit Effects
let gold = 1000;
...
function saveGame() {
    const data = { 
        ...
        slotUpgrades, // 제련 데이터 저장
        lastTime: Date.now(), isMiningPaused 
    };
    localStorage.setItem('toothSaveV240', JSON.stringify(data));
}

function loadGame() {
    const saved = localStorage.getItem('toothSaveV240');
    if (saved) {
    ...
    updateSoundBtn();
    updatePickaxeVisual(); // 곡괭이 비주얼 업데이트
}
...
function switchView(view) {
    currentView = view;
    // 탭 3개 관리
    document.getElementById('mine-view').style.display = (view === 'mine' || view === 'refine') ? 'flex' : 'none'; // 광산은 제련탭에서도 보임 (선택사항이나 공간상 mine에만 두자)
    if(view === 'refine') document.getElementById('mine-view').style.display = 'none'; // 제련탭에선 광산 숨김
...
function renderRefineView() {
    ...
    slotUpgrades.forEach((slot, idx) => {
        ...
        card.innerHTML = `
            <div class="refine-header">🔥 슬롯 #${idx+1} 강화</div>
            <div class="refine-btn" onclick="upgradeSlot(${idx}, 'atk', ${costAtk})">
                <span>⚔️ 공격 Lv.${slot.atk}</span> <span>💰${fNum(costAtk)}</span>
            </div>
            <div class="refine-btn" onclick="upgradeSlot(${idx}, 'cd', ${costCd})">
                <span>⏳ 쿨타임 Lv.${slot.cd}</span> <span>💰${fNum(costCd)}</span>
            </div>
            <div class="refine-btn" onclick="upgradeSlot(${idx}, 'rng', ${costRng})">
                <span>🏹 사거리 Lv.${slot.rng}</span> <span>💰${fNum(costRng)}</span>
            </div>
        `;
...
function setupMiningTouch() {
    ...
    mineArea.addEventListener('pointerdown', (e) => {
        ...
        miner.style.animation = 'none'; miner.offsetHeight; miner.style.animation = 'hammer 0.08s ease-in-out';
        
        playSfx('mine');
        processMining(15); 
        createHitEffect(e.clientX, e.clientY); // 타격 효과
    });
}
...
function updatePickaxeVisual() {
    const pick = TOOTH_DATA.pickaxes[pickaxeIdx];
    document.getElementById('miner-char').innerText = pick.icon || "⛏️"; // data.js의 아이콘 사용
}
...
function buyItem(type, cost) {
    if (gold >= cost) {
        ...
        if (type === 'pick') {
            ...
            sortInventory();
            updatePickaxeVisual(); // 업데이트 시 비주얼 변경
        } 
...
// ... (기본 함수들: save/load/renderInventory 등 v2.2.3 동일) ...
...
function exportSave() { saveGame(); const data = localStorage.getItem('toothSaveV240'); const encoded = btoa(unescape(encodeURIComponent(data))); prompt("코드 복사:", encoded); }
function importSave() { const str = prompt("코드 붙여넣기:"); if (str) { try { const decoded = decodeURIComponent(escape(atob(str))); localStorage.setItem('toothSaveV240', decoded); location.reload(); } catch (e) { alert("오류"); } } }
