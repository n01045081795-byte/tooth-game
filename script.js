// Version: 2.2.0 - Fixed Layout & Logic Improvements
let gold = 1000;
...
function saveGame() {
    ...
    localStorage.setItem('toothSaveV220', JSON.stringify(data));
}

function loadGame() {
    const saved = localStorage.getItem('toothSaveV220');
    if (saved) {
...
// 스마트 자동 합성 (1개 남은 것은 건너뜀)
function autoMergeLowest() {
    // 1. 존재하는 모든 레벨을 찾아서 개수를 셉니다.
    let levelCounts = {};
    ...
    // 2. 가장 낮은 레벨부터 확인하되, 개수가 2개 이상인 것만 찾습니다.
    let targetLv = -1;
    ...
    // 3. 합성 가능한 레벨을 찾았다면 합성 실행
    if (targetLv !== -1) {
        massMerge(targetLv, true); // true = 1회(한 쌍)만 합성
    }
}
...
function massMerge(lv, once = false) {
    ...
    // once가 true면 한 쌍만, false면 가능한 전부
...
function addMinedItem(pick) {
    ...
    // 빈칸이 꽉 찼으면, 잠금해제된 칸 범위 내에서 다시 확인
    let realEmpty = -1;
    for(let i=0; i<maxSlots; i++) {
        if(inventory[i] === 0) { realEmpty = i; break; }
    }

    if (realEmpty === -1) {
        autoMergeLowest(); // 공간 확보 시도
        // 다시 확인
        for(let i=0; i<maxSlots; i++) {
            if(inventory[i] === 0) { realEmpty = i; break; }
        }
    ...
    if (realEmpty !== -1) {
        const isGreat = Math.random() < pick.greatChance;
        inventory[realEmpty] = isGreat ? pick.mineLv + 1 : pick.mineLv;
        if(currentView === 'mine') renderInventory();
        if(isGreat && currentView === 'mine') triggerGreatSuccess(realEmpty);
    }
}

function gameLoop() {
    if(!isMiningPaused) {
        // 채굴 속도 너프 적용 (0.05 -> 0.02)
...
function renderInventory() {
    ...
    for (let i = 0; i < 64; i++) {
        ...
        if (i < maxSlots) {
            ...
            slot.ondblclick = (e) => { e.stopPropagation(); if(inventory[i] > 0) massMerge(inventory[i]); };
        }
        grid.appendChild(slot);
    }
}

// (나머지 moveProxy, handleMoveOrMerge, triggerGreatSuccess, UI 함수들은 이전과 동일)
...
function exportSave() { saveGame(); const data = localStorage.getItem('toothSaveV220'); const encoded = btoa(unescape(encodeURIComponent(data))); prompt("코드 복사:", encoded); }
function importSave() { const str = prompt("코드 붙여넣기:"); if (str) { try { const decoded = decodeURIComponent(escape(atob(str))); localStorage.setItem('toothSaveV220', decoded); location.reload(); } catch (e) { alert("오류"); } } }
// Battle & Render Lists (from v2.1.4) - 그대로 사용
