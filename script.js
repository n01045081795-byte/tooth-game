// Version: 2.1.3 - Smart Mining & Layout Fix
let gold = 1000;
...
function saveGame() {
    ...
    localStorage.setItem('toothSaveV213', JSON.stringify(data));
}

function loadGame() {
    const saved = localStorage.getItem('toothSaveV213');
    if (saved) {
...
function addMinedItem(pick) {
    ...
    // 빈칸이 없으면 가장 낮은 레벨을 찾아 자동 합성 시도
    if (emptyIdx === -1 || emptyIdx >= maxSlots) {
        autoMergeLowest();
        emptyIdx = inventory.indexOf(0); // 다시 확인
    }

    // 그래도 빈칸이 있고 잠금 해제된 슬롯이라면 추가
    if (emptyIdx !== -1 && emptyIdx < maxSlots) {
        const isGreat = Math.random() < pick.greatChance;
        inventory[emptyIdx] = isGreat ? pick.mineLv + 1 : pick.mineLv;
        if(currentView === 'mine') renderInventory(); // 성능 위해 뷰 체크
    }
}
...
function autoMergeLowest() {
    ...
    for(let i=0; i<maxSlots; i++) {
        if(inventory[i] > 0 && inventory[i] < minLv) {
            minLv = inventory[i];
            found = true;
        }
    }
    
    if(found) massMerge(minLv);
}
...
function processMining(amt) {
    ...
    if (mineProgress >= 100) {
        mineProgress = 0;
        addMinedItem(pick); // 스마트 추가 함수 호출
        
        // 대성공 이펙트 (현재 화면에 보이는 경우만)
        if (currentView === 'mine') {
            let lastIdx = -1; 
            for(let i=0; i<maxSlots; i++) { if(inventory[i]>0) lastIdx=i; } // 대략적인 위치
            // 정확한 위치 찾기는 복잡하므로 패스하거나 단순화
        }
    }
    updateUI();
}

// --- (이하 기존 로직 유지: 드래그, 합성, 상점, 쿠폰 등) ---
...
function switchView(view) {
    ...
    if (view === 'war') { renderDungeonList(); renderMercenaryCamp(); } else { renderInventory(); }
}

function checkCoupon() {
    const code = document.getElementById('coupon-input').value.trim();
    if (code === "100b" || code === "RICH100B") {
        gold += 100000000000; alert("치트키 적용: 100b 골드 획득!"); updateUI();
    } else { alert("유효하지 않은 쿠폰입니다."); }
}

function exportSave() {
    saveGame();
    const data = localStorage.getItem('toothSaveV213');
...
function importSave() {
    ...
    if (str) {
        try {
            ...
            localStorage.setItem('toothSaveV213', decoded);
            location.reload();
        } catch (e) { alert("잘못된 저장 코드입니다."); }
    }
}
...
function renderDungeonList() {
    ...
    TOOTH_DATA.dungeons.forEach((name, idx) => {
        ...
        if (isUnlocked) { div.innerHTML = `<h4>⚔️ ${name}</h4><p>권장 Lv.${idx + 1} 이상</p>`; div.onclick = () => startDungeon(idx); } 
        else { div.innerHTML = `<h4>🔒 잠김</h4><p>Lv.${idx} 클리어 시 열림</p>`; }
...
function renderMercenaryCamp() {
    ...
    TOOTH_DATA.mercenaries.forEach(merc => {
        ...
        else if (isOwned) div.innerHTML += `<button onclick="equipMerc(${merc.id})" class="btn-sm">장착</button>`;
        else div.innerHTML += `<button onclick="buyMerc(${merc.id}, ${merc.cost})" class="btn-gold" style="padding:2px 5px; font-size:10px;">${fNum(merc.cost)}G</button>`;
