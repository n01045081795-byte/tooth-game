// Version: 3.3.0 - Script with Dungeon Info
// ... (변수 선언 동일) ...
let gold = 1000;
...
function saveGame() {
    ...
    localStorage.setItem('toothSaveV330', JSON.stringify(data));
}

function loadGame() {
    const saved = localStorage.getItem('toothSaveV330');
    if (saved) {
...
function renderDungeonList() {
    ...
    TOOTH_DATA.dungeons.forEach((name, idx) => {
        ...
        if (isUnlocked) {
            // 보상 레벨: idx+2 (ex: 0번 던전(1탄) 깨면 2레벨 치아)
            div.innerHTML = `
                <h4>⚔️ Lv.${idx+1} ${name}</h4>
                <p>권장 공격력: Lv.${idx+1}0+</p>
                <p style="color:#f1c40f; font-size:10px;">클리어 시: Lv.${idx+2} 치아 확정 채굴</p>
            `;
...
// ... (나머지 로직들은 v3.2.0과 동일, index/style이 중요함) ...
...
function addMinedItem(pick) { let emptyIdx = -1; for(let i=0; i<maxSlots; i++) { if(inventory[i] === 0) { emptyIdx = i; break; } } if (emptyIdx === -1) { autoMergeLowest(); for(let i=0; i<maxSlots; i++) { if(inventory[i] === 0) { emptyIdx = i; break; } } } if (emptyIdx !== -1) { const baseLv = Math.max(pick.mineLv, unlockedDungeon); const isGreat = Math.random() < pick.greatChance; inventory[emptyIdx] = isGreat ? baseLv + 1 : baseLv; if(currentView === 'mine') renderInventory(); if(isGreat && currentView === 'mine') triggerGreatSuccess(emptyIdx); } }
...
function exportSave() { saveGame(); const data = localStorage.getItem('toothSaveV330'); const encoded = btoa(unescape(encodeURIComponent(data))); prompt("코드 복사:", encoded); }
function importSave() { const str = prompt("코드 붙여넣기:"); if (str) { try { const decoded = decodeURIComponent(escape(atob(str))); localStorage.setItem('toothSaveV330', decoded); location.reload(); } catch (e) { alert("오류"); } } }
