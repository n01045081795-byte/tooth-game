// Version: 3.2.0 - Gauge & Refine Fixes
let gold = 1000;
...
function saveGame() {
    ...
    localStorage.setItem('toothSaveV320', JSON.stringify(data));
}

function loadGame() {
    const saved = localStorage.getItem('toothSaveV320');
    if (saved) {
...
// ... (기존 기능 로직 유지: renderInventory, switchView, etc.) ...
...
function exportSave() { saveGame(); const data = localStorage.getItem('toothSaveV320'); const encoded = btoa(unescape(encodeURIComponent(data))); prompt("코드 복사:", encoded); }
function importSave() { const str = prompt("코드 붙여넣기:"); if (str) { try { const decoded = decodeURIComponent(escape(atob(str))); localStorage.setItem('toothSaveV320', decoded); location.reload(); } catch (e) { alert("오류"); } } }
...
function manualMine() {} // handled by touch
