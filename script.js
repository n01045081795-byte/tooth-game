...
// 자동 채굴/합성 만렙 설정
const MAX_AUTO_MINE_LV = 40; // 40 * 0.2 = 8초 단축 (15-8=7초)
const MAX_AUTO_MERGE_LV = 15; // 15 * 1 = 15초 단축 (25-15=10초)
...
function renderShopItems() {
    ...
    if (pickNext) {
        ...
        container.innerHTML += `
        <div class="shop-item">
            <div class="shop-info"><span>⚒️ ${pick.name}</span> <button class="btn-max">MAX</button></div>
...
function buyItem(type, cost) {
    if (gold >= cost) {
        if (type === 'pick') {
            if (!TOOTH_DATA.pickaxes[pickaxeIdx + 1]) return; // 이미 만렙
            gold -= cost;
            pickaxeIdx++;
...
function exportSave() { saveGame(); const data = localStorage.getItem('toothSaveV370'); const encoded = btoa(unescape(encodeURIComponent(data))); prompt("코드 복사:", encoded); }
function importSave() { const str = prompt("코드 붙여넣기:"); if (str) { try { const decoded = decodeURIComponent(escape(atob(str))); localStorage.setItem('toothSaveV370', decoded); location.reload(); } catch (e) { alert("오류"); } } }
